...
import { DEFAULT_FONT_SIZE, DEFAULT_ALIGNMENT, VERTICAL_ALIGN_TOP, VERTICAL_ALIGN_MIDDLE, VERTICAL_ALIGN_BOTTOM, DEFAULT_VERTICAL_ALIGNMENT, DEFAULT_LINE_HEIGHT, DEFAULT_CHARACTER_SPACING, DEFAULT_FONT_COLOR, BCP_POSITIVE_BACKGROUND_COLOR, BCP_NEGATIVE_BACKGROUND_COLOR, } from './constants.js';
import { calculateDynamicFontSize, getFontKitFont, getBrowserVerticalFontAdjustments, isFirefox, checkBcps, formatBcpText, } from './helper.js';
...
export const uiRender = async (arg) => {
    ...
    console.log("in uiRender inputs:", inputs);
    const variables = inputs && inputs.length > 0 ? inputs[0] : undefined;
    console.log("in uiRender:", variables);
    ...
    const displayText = formatBcpText(value, variables, schema.bcpField, schema.debug);
    // Determine background color based on BCP count
    const { hasBcps } = checkBcps(value, variables, schema.bcpField, schema.debug);
    const bgColor = hasBcps ? BCP_POSITIVE_BACKGROUND_COLOR : BCP_NEGATIVE_BACKGROUND_COLOR;
    const textBlock = buildStyledTextContainer(arg, fontKitFont, displayText, bgColor);
    const processedText = replaceUnsupportedChars(displayText, fontKitFont);
    if (!isEditable(mode, schema)) {
        // Read-only mode
        textBlock.innerHTML = processedText
            .split('')
            .map((l, i) => `<span style="letter-spacing:${processedText.length === i + 1 ? 0 : 'inherit'};">${l}</span>`)
            .join('');
        return;
    }
...
export const buildStyledTextContainer = (arg, fontKitFont, value, backgroundColor) => {
