...
// Enhanced version of checkBcps with debugging
export const checkBcps = (
    value, // The control's own value
    variables, // All available variables
    bcpField, // Field to check for BCP count
    debug = false // Debug flag
) => {
    // Debug logging function
    const log = (message, data) => {
        if (debug) {
            console.log(`[numOfBcps] ${message}`, data);
        }
    };

    log("Function called with:", { value, bcpField, variablesKeys: variables ? Object.keys(variables) : 'none' });
    ...
    if (bcpField && bcpField.startsWith('{') && bcpField.endsWith('}')) {
        ...
        log("Looking for direct variable reference:", variableName);
        
        if (variables && variables[variableName] !== undefined) {
            const rawValue = variables[variableName];
            log("Found variable value:", rawValue);
            
            // Handle various formats: number, string, boolean
            let numericValue;
            if (typeof rawValue === 'number') {
                numericValue = rawValue;
            } else if (typeof rawValue === 'string') {
                // Try to parse as number, handling comma-formatted numbers
            ...
            log("Converted to numeric value:", numericValue);
            
            if (!isNaN(numericValue)) {
                const result = { hasBcps: numericValue > 0, count: numericValue };
                log("Returning result:", result);
                return result;
            }
        }
        log("Variable not found or not numeric");
        return { hasBcps: false, count: 0 }; // Variable not found or not numeric
    }

    // Check if the control's own value is a number and use it if it is
    const ownNumericValue = parseFloat(String(value).replace(/,/g, ''));
    log("Control's own value is a number:", ownNumericValue);
    log("what is the value:", String(value));
    if (!isNaN(ownNumericValue)) {
        log("Control's own value is a number:", ownNumericValue);
        // If no bcpField is provided or no variables are available, use the control's own value
        if (!bcpField || !variables) {
            log("Using control's own value (no bcpField or no variables)");
            const result = {
                hasBcps: ownNumericValue > 0,
                count: ownNumericValue
            };
            log("Returning result:", result);
            return result;
        }
    } else {
        log("Control's own value is not a number:", value);
        // If no bcpField is provided or no variables are available, return no BCPs
        if (!bcpField || !variables) {
            log("No bcpField or no variables, and own value is not a number");
            return { hasBcps: false, count: 0 };
        }
    }

    // Try to find the field value by direct lookup
    let fieldValue = variables[bcpField];
    log("Direct lookup result:", fieldValue);
    ...
    if (fieldValue === undefined) {
        ...
        const possibleKeys = [
            bcpField, // Exact match
            `field_${bcpField}`, // field_name format
            `${bcpField}_field`, // name_field format
            bcpField.toLowerCase(), // lowercase
            bcpField.toUpperCase(), // uppercase
        ];
        
        log("Trying alternative key formats:", possibleKeys);
        
        // Check all possible keys
        for (const key of possibleKeys) {
            if (variables[key] !== undefined) {
                fieldValue = variables[key];
                log("Found value using key:", { key, value: fieldValue });
                break;
            }
        }
        
        // If still not found, check if any key ends with the bcpField
        if (fieldValue === undefined) {
            log("Checking for keys ending with bcpField");
            for (const [key, val] of Object.entries(variables)) {
                if (key.endsWith(`_${bcpField}`)) {
                    fieldValue = val;
                    log("Found value using key ending:", { key, value: fieldValue });
                    break;
                }
            }
        }
        
        // If still not found, check if any key contains the bcpField
        if (fieldValue === undefined) {
            log("Checking for keys containing bcpField");
            for (const [key, val] of Object.entries(variables)) {
                if (key.includes(bcpField)) {
                    fieldValue = val;
                    log("Found value using key containing:", { key, value: fieldValue });
                    break;
                }
            }
        }
    ...
    // If we found a field value, check if it's greater than 0
    if (fieldValue !== undefined) {
        log("Processing found field value:", fieldValue);
        ...
        if (typeof fieldValue === 'number') {
            ...
            // Try to parse as number, handling comma-formatted numbers
            numericValue = parseFloat(fieldValue.replace(/,/g, ''));
    ...
    log("Falling back to control's own value:", value);
...
export const formatBcpText = (
    ...
    const { hasBcps, count } = checkBcps(value, variables, bcpField, debug);
    if (hasBcps) {
        return `BCPs: ${count}`;
    }
    else {
