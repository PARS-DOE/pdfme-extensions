...
export const isAboveThreshold = (value, threshold, variables, thresholdField) => {
    console.log('🔍 isAboveThreshold DETAILED ANALYSIS:');
    console.log('📥 Input parameters:', { 
        value, 
        valueType: typeof value,
        threshold, 
        thresholdType: typeof threshold,
        variablesAvailable: variables ? true : false,
        variablesKeys: variables ? Object.keys(variables) : 'none', 
        thresholdField,
        thresholdFieldType: typeof thresholdField
    });
    
    // IMPORTANT: Since we're setting the content value directly in PDFViewer.tsx,
    // we should prioritize using the control's own value (which is already set to the field value)
    // This fixes the issue where variables is empty but we still want to use the correct value
    
    // Special case for non-numeric string values like 'Other'
    if (typeof value === 'string' && (value === 'Other' || value === 'N/A' || value === '-')) {
        console.log('📝 Special handling for non-numeric value:', value);
        
        // If we have a thresholdField and variables, try to get the actual numeric value
        if (thresholdField && variables && Object.keys(variables).length > 0) {
            const fieldName = thresholdField.replace(/^\{|\}$/g, '');
            if (variables[fieldName] !== undefined) {
                const fieldValue = variables[fieldName];
                const fieldNumericValue = parseFloat(fieldValue);
                if (!isNaN(fieldNumericValue)) {
                    console.log('📝 Found numeric value in variables:', fieldNumericValue);
                    const result = fieldNumericValue > threshold;
                    console.log('⚖️ Variable value comparison:', fieldNumericValue, 'threshold:', threshold, 'result:', result);
                    return result;
                }
            }
        }
        
        // Default behavior for non-numeric values: treat as below threshold
        console.log('⚖️ Treating non-numeric value as BELOW threshold');
        return false;
    }
    
    // First, try to use the control's own value directly
    const numericValue = parseFloat(value);
    if (!isNaN(numericValue)) {
        console.log('📝 Using control\'s own value (direct):', value);
        // FIXED: Check if value is GREATER THAN threshold (not less than or equal to)
        // This matches the expected behavior in the adapters.ts file
        const result = numericValue > threshold;
        console.log('⚖️ Direct value comparison:', numericValue, 'threshold:', threshold, 'result:', result, `(${numericValue} > ${threshold} = ${result})`);
        return result;
    }
    
    // If the control's value isn't numeric, then try the variables approach as fallback
    
    // Dump all variables for inspection
    if (variables) {
    ...
    if (thresholdField && thresholdField.startsWith('{') && thresholdField.endsWith('}')) {
        ...
        console.log('🔎 Looking for direct variable reference:', variableName);
        
        if (variables && variables[variableName] !== undefined) {
            const rawValue = variables[variableName];
            const varNumericValue = parseFloat(rawValue);
            console.log('✅ Found variable value:', rawValue, 'parsed as:', varNumericValue, 'threshold:', threshold);
            const result = !isNaN(varNumericValue) && varNumericValue > threshold;
            console.log('⚖️ Comparison result:', result, `(${varNumericValue} > ${threshold} = ${result})`);
            return result;
        }
        console.log('❌ Variable not found or not numeric');
    }
    
    // If no thresholdField is provided or variables is empty
    if (!thresholdField || !variables) {
        console.log('📝 Using control\'s own value (fallback):', value);
        // We already tried parsing the value above, and it wasn't numeric
        console.log('⚖️ Value is not numeric, returning false');
        return false;
    }
    // Try to find the field value by direct lookup
    console.log('🔍 Trying direct lookup with thresholdField:', thresholdField);
    let fieldValue = variables[thresholdField];
    console.log('📊 Direct lookup result:', fieldValue);
    ...
    if (fieldValue === undefined) {
        ...
        console.log('🔍 Trying alternative key formats:', possibleKeys);
        
        // Check all possible keys
        for (const key of possibleKeys) {
            if (variables[key] !== undefined) {
                fieldValue = variables[key];
                console.log('✅ Found value using key:', key, 'value:', fieldValue);
                break;
            }
        }
        ...
        if (fieldValue === undefined) {
            console.log('🔍 Checking for keys ending with thresholdField');
            for (const [key, val] of Object.entries(variables)) {
                if (key.endsWith(`_${thresholdField}`)) {
                    fieldValue = val;
                    console.log('✅ Found value using key ending:', key, 'value:', fieldValue);
                    break;
                }
            }
        }
    ...
    if (fieldValue !== undefined) {
        console.log('📊 Processing found field value:', fieldValue);
        const fieldNumericValue = parseFloat(fieldValue);
        const result = !isNaN(fieldNumericValue) && fieldNumericValue > threshold;
        console.log('⚖️ Field value parsed as:', fieldNumericValue, 'threshold:', threshold, 'result:', result, `(${fieldNumericValue} > ${threshold} = ${result})`);
        return result;
    }
    
    // Otherwise, fall back to using the current field's value
    console.log('⚠️ Field value not found, falling back to control\'s own value:', value);
    const fallbackNumericValue = parseFloat(value);
    const result = !isNaN(fallbackNumericValue) && fallbackNumericValue > threshold;
    console.log('⚖️ Fallback value parsed as:', fallbackNumericValue, 'threshold:', threshold, 'result:', result, `(${fallbackNumericValue} > ${threshold} = ${result})`);
    return result;
};
