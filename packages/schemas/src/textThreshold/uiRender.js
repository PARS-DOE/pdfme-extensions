...
import { calculateDynamicFontSize, getFontKitFont, getBrowserVerticalFontAdjustments, isFirefox, isAboveThreshold, } from './helper.js';
...
const getBackgroundColor = (value, schema, variables) => {
    console.log('🎨 getBackgroundColor DETAILED ANALYSIS:');
    console.log('📥 Input parameters:', { 
        value, 
        valueType: typeof value,
        content: schema.content,
        contentType: typeof schema.content,
        threshold: schema.threshold, 
        thresholdType: typeof schema.threshold,
        thresholdField: schema.thresholdField,
        thresholdBackgroundColor: schema.thresholdBackgroundColor,
    ...
    // IMPORTANT: Use schema.content for threshold comparison instead of value
    // This fixes the issue where the control is using the displayed value "Active"
    // instead of the content value "5781000000" for the threshold comparison
    const valueToCheck = schema.content || value;
    
    if (!valueToCheck) {
        console.log('❌ No value to check, returning transparent');
        return 'transparent';
    }
    
    // Check threshold condition using the helper function
    if (schema.threshold !== undefined && schema.thresholdBackgroundColor) {
        console.log('⚖️ Checking threshold condition...');
        
        // Log the actual numeric value we're comparing
        const numericValue = parseFloat(valueToCheck);
        console.log(`📊 Numeric value from content: ${numericValue}, Threshold: ${schema.threshold}, Is above? ${numericValue > schema.threshold}`);
        
        const isAboveThresholdValue = isAboveThreshold(valueToCheck, schema.threshold, variables, schema.thresholdField);
        console.log('⚖️ isAboveThreshold result:', isAboveThresholdValue);
        
        // If the value is below threshold, apply the threshold background color
        if (!isAboveThresholdValue) {
            console.log('🔴 Value is NOT above threshold, applying thresholdBackgroundColor:', schema.thresholdBackgroundColor);
            return schema.thresholdBackgroundColor;
        } else {
            console.log('🟢 Value IS above threshold, using backgroundColor or transparent');
        }
    } else {
        console.log('⚠️ No threshold or thresholdBackgroundColor defined');
    }
    
    const result = schema.backgroundColor || 'transparent';
    console.log('🎨 Returning backgroundColor or transparent:', result);
    return result;
};
